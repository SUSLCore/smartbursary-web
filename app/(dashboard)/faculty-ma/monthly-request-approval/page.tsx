...
export default function MonthlyRequestApprovalPage() {
  ...
  const loadStudents = useCallback(async (departmentIdValue: number) => {
    if (!departmentIdValue) {
      setStudentNotice({
        tone: "error",
        text: "Please choose a department before loading students.",
      });
      return;
    }
    ...
    try {
      ...
      setStudentNotice({
        tone: "success",
        text: `Loaded ${response.count ?? response.data?.length ?? 0} eligible students for department ${departmentIdValue}.`,
      });
    } catch (error) {
      console.error(error);
      setStudentNotice({
  ...
  useEffect(() => {
    ...
    void loadStudents(parsedDepartmentId);
  }, [loadStudents, selectedDepartmentId]);
  ...
  const handleUpload = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await loadStudents(parsedDepartmentId);
    } catch (error) {
      console.error(error);
      setUploadNotice({
        tone: "error",
        ...
        <div className="grid gap-0 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
          <div className="p-6 sm:p-8">
            ...
            <h1 className="mt-4 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
              Eligible student list manager
            </h1>
            <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">
              Upload the eligible-students workbook, inspect department
              allocations, check a single register ID, and remove entries when
              the monthly request list changes.
            </p>
          </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="space-y-6">
          ...
          >
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  The batch and department are loaded from the backend. Pick
                  the registered values and upload the Excel file.
                </p>
              </div>
            ...
            >
              {uploading ? "Uploading..." : "Upload eligible student list"}
            </button>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-600">
                  The student list is tied to the selected department and can be
                  refreshed anytime.
                </p>
              </div>
              ...
              <button
                ...
                disabled={loadingStudents || !selectedDepartmentId}
              ...
              >
                {loadingStudents ? "Loading..." : "Refresh students"}
              </button>
            </div>
                  ...
                  <tbody className="divide-y divide-slate-200 bg-white">
                    {students.length === 0 ? (
                      <tr>
                        ...
                        >
                          No students loaded yet. Select a department to load
                          its eligible list.
                        </td>
                      </tr>
                    ) : (
        ...
        <aside className="space-y-6">
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Enter a register ID to verify whether the student is already in
              the eligible list.
            </p>
          ...
          <section className="rounded-3xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-600">
              <li className="rounded-2xl bg-white px-4 py-3 ring-1 ring-slate-200">
                The upload endpoint expects `file`, `batchId`, `facultyId`, and
                `departmentId`.
              </li>
              <li className="rounded-2xl bg-white px-4 py-3 ring-1 ring-slate-200">
                Batch and department values now come from the backend, so the
                form always uses registered records.
              </li>
              <li className="rounded-2xl bg-white px-4 py-3 ring-1 ring-slate-200">
                Refresh the department list if you need to confirm the final
